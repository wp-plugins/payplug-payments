<?php

require_once(__DIR__ . "/../lib/Payplug.php");

class ParametersTest extends PHPUnit_Framework_TestCase {

    protected static $filePath;
    protected static $payplugPublicKey;
    protected static $privateKey;

    protected static $parameters;

    public static function setUpBeforeClass() {
        self::$filePath = __DIR__ . "/parameters.json";
        self::$payplugPublicKey = "-----BEGIN PUBLIC KEY-----\\nzQIDAQAB\n-----END PUBLIC KEY-----\n";
        self::$privateKey = "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----";
        self::$parameters = new Parameters(
            array("EUR"),
            5000,
            1,
            "https://www.payplug.fr/p/MD1iqp-NEeO1vBIxQwfJEg==",
            self::$payplugPublicKey,
            self::$privateKey
        );
    }

    public static function tearDownAfterClass() {
        unlink(self::$filePath);
    }

    public function testToString() {
        $str = self::$parameters->toString();

        $this->assertNotNull($str);
        $this->assertTrue(strlen($str) > 0);

        return $str;
    }

    /**
     * @depends testToString
     */
    public function testSaveInFile($str) {
        self::$parameters->saveInFile(self::$filePath);

        $this->assertFileExists(self::$filePath);
        $this->assertEquals($str, file_get_contents(self::$filePath));
    }

    /**
     * @depends testToString
     */
    public function testCreateFromString($str) {
        $parameters2 = Parameters::createFromString($str);

        $this->assertNotNull($parameters2);
        $this->assertInstanceOf("Parameters", $parameters2);
        $this->assertEquals(self::$parameters, $parameters2);
    }

    /**
     * @depends testSaveInFile
     */
    public function testLoadFromFile() {
        $parameters2 = Parameters::loadFromFile(self::$filePath);

        $this->assertNotNull($parameters2);
        $this->assertInstanceOf("Parameters", $parameters2);
        $this->assertEquals(self::$parameters, $parameters2);
    }
}

